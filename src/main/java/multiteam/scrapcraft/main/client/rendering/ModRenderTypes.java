...
public class ModRenderTypes extends RenderType {
    ...
    public static final RenderType CONNECTION_POINT = RenderType.create("connection_point", DefaultVertexFormats.POSITION_COLOR, 7, 262144,  RenderType.State.builder().setLayeringState(VIEW_OFFSET_Z_LAYERING).setShadeModelState(SMOOTH_SHADE).setLightmapState(NO_LIGHTMAP).setTransparencyState(TRANSLUCENT_TRANSPARENCY).setOutputState(TRANSLUCENT_TARGET).setDepthTestState(RenderState.NO_DEPTH_TEST).setCullState(NO_CULL).setWriteMaskState(RenderState.COLOR_WRITE).createCompositeState(true));
